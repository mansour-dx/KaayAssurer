...
<div class="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
    <div class="px-4 py-5 sm:px-6 bg-green-500 text-white">
        <h3 class="text-lg leading-6 font-medium">Enregistrer un Paiement</h3>
        <p class="mt-1 max-w-2xl text-sm">Ajouter un nouveau paiement au système</p>
    </div>
</div>

<div class="bg-white shadow overflow-hidden sm:rounded-lg">
    <form action="{{ route('paiements.store') }}" method="POST" class="p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <input type="date" name="date_paiement" id="date_paiement" value="{{ old('date_paiement', date('Y-m-d')) }}" class="mt-1 focus:ring-green-500 focus:border-green-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                @error('date_paiement')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="montant" id="montant" value="{{ old('montant') }}" class="mt-1 focus:ring-green-500 focus:border-green-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                @error('montant')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="methode_paiement" id="methode_paiement" class="mt-1 focus:ring-green-500 focus:border-green-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    ...
                    <option value="prelevement" {{ old('methode_paiement') == 'prelevement' ? 'selected' : '' }}>Prélèvement automatique</option>
                </select>
            ...
            <div>
                ...
                <input type="text" name="reference_transaction" id="reference_transaction" value="{{ old('reference_transaction') }}" class="mt-1 focus:ring-green-500 focus:border-green-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
